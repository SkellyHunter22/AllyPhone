...
public class AlertsApp implements PhoneApp {
    ...
    public ItemStack getIcon(Player viewer) {
        ...
        return GuiUtil.icon(Material.BELL, getDisplayName() + badge, "§7System status & warnings");
    }
    ...
    public void open(Player player) {
        ...
        inv.setItem(0, GuiUtil.icon(signal > 0 ? Material.LIME_DYE : Material.RED_DYE,
                signal > 0 ? "§aSignal OK (" + signal + "/5)" : "§cNo Signal",
                "§7You are " + (signal > 0 ? "in" : "out of") + " coverage."));

        inv.setItem(1, GuiUtil.icon(active ? Material.EMERALD : Material.BARRIER,
                active ? "§aBilling Active" : "§cService Suspended",
                active ? "§7Your account is in good standing." : "§7Pay your bill to restore service."));

        inv.setItem(2, GuiUtil.icon(Material.BEACON, "§bServer Online",
        ...
        if (alerts.isEmpty()) {
            inv.setItem(13, GuiUtil.icon(Material.PAPER, "§7No alerts", "§8Nothing to report right now."));
        } else {
            int slot = 9;
            for (AlertSQLService.StoredAlert alert : alerts) {
                if (slot >= 27) break;
                inv.setItem(slot, GuiUtil.icon(
                        alert.read() ? Material.PAPER : Material.WRITABLE_BOOK,
