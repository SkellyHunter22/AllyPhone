...
public class PetsApp implements PhoneApp {
    ...
    public ItemStack getIcon(Player viewer) {
        return GuiUtil.icon("buy_manage_pets", getDisplayName(), "§7Buy & manage pets");
    }
    ...
    public void open(Player player) {
        ...
        if (present) {
            inv.setItem(10, GuiUtil.tagged(plugin,
                    GuiUtil.icon("buy_manage_pets", "§eBuy & Manage Pets", "§7Click to open GPet"),
                    GuiUtil.ACTION_KEY, "command:gpet gui"));
            inv.setItem(13, GuiUtil.tagged(plugin,
                    GuiUtil.icon("my_pets", "§eMy Pets", "§7List your pets by ID", "§7and hide the ones you don't want out"),
                    GuiUtil.ACTION_KEY, "mypets"));
            inv.setItem(16, GuiUtil.tagged(plugin,
                    GuiUtil.icon("hide_pet", "§eHide Pet", "§7Despawns your active pet",
                            "§7You still own it and can respawn it",
                            "§7any time, keeping its name and data."),
                    GuiUtil.ACTION_KEY, "command:gpet remove"));
        } else {
            inv.setItem(13, GuiUtil.icon("gpet_is_not_installed", "§cGPet is not installed"));
        }
